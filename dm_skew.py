...
class dm_skew_2(distance_metric):
  ...
  def vector_distances(self, v1, v2, thresh=10):
    ...
    a = self.alpha
    sum_ratio = (sum1.astype(float) / sum2.transpose()) # POSSIBLE DIV BY ZERO
    acc_a = numpy.empty((v1.shape[0], v2.shape[0]))
...
class dm_skew_3(distance_metric):
  # Identical to 1, except for a small tweak to avoid producing extra boolean arrays
  # by returning some of the outer looping to python space
  __name__ = "skew_3"
